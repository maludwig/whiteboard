...
var menu = {
	initialize: function() {
		...
		//Palettes
		$.get("js/palettes.json",function(pals) {
			var pal;
			var $pal;
			var $firstpal;
			var $firstcolor;
		...
		for(var key in sizes) {
			i = sizes[key];
			$sz = $('<div class="size" id="size' + i + '"></div>');
		...
		$(".sizewrap").touchStart(function() {
			...
			scratch.strokeWidth($(this).find(".size").data("size"));
			scratchFlow = new Flow({surface:scratch});
			menu.setSizeMargins();
		});
        //Initial size of 10px
        $("#size10").addClass("active");
        scratch.strokeWidth(10);
        scratchFlow = new Flow({surface:scratch});
